...
public class DB : IModel
{
    ...
    public DPoint[] GetCurvePoints()
    {
        return CurvePoints;
    }

    public DPoint[] GetStraightPoint()
    {
        return StraightPoints;
    }

    public AreaDeviation[] GetAreaDeviations()
    {
        return maxLocalAreaDeviations ?? new AreaDeviation[0];
    }
    ...
    public void UpdateMeterDeflectionAllDataList()
    {
        for (var i = 1; i < DataList.Count; i++)
        {
            ...
            if (DataList.Count - i < _stepsPerMeter) DataList[i].DeviationPerMeter = 0;
        }
    }
    ...
    {
        return GetY(DataList[index - 1].Position,
            DataList[index - 1].FactProfile,
            DataList[index].Position,
            DataList[index].FactProfile,
            coord);
    }
    ...
    public AreaDeviation GetAreaDeviation(int startPos)
    {
        var startX = startPos;
        decimal startY;
        var endX = startX + LocalAreaLength;
        decimal endY;
    ...
    public (int startX, int endX, decimal delta) GetDeltaAreaDeviation(int startInteval, int endInterval,
        List<(int x, decimal y)> LocalAreaStraight)
    {
    ...
    public void UpdateRow(int index, int value, Direction direction, Units unit,
        AngleUnits angleUnits = AngleUnits.Second) //int index, int value)
    {
        if (index > 0)
            switch (unit)
            {
    ...
    public (string[][] dbValues, string[][] dataListValues) GetPrintLists()
    {
        ...
        [
            ...
            [
                "Отклонение от прямолинейности в вертикальной плоскости, мкм",
                Math.Round(_verticalDeflection, 2).ToString()
            ],
    ...
    public (double[] positions, double[] graph1, double[] graph2) GetGraphicPoints()
    {
        ...
        return new ValueTuple<double[], double[], double[]>(pos, graph1, graph2);
    }
    ...
    public decimal GetAreaDeflection()
    {
        return _localAreaDeflection;
    }
